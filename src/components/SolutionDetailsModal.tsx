...
export const SolutionDetailsModal: React.FC<SolutionDetailsModalProps> = ({
  ...
  const currentStatus = submission.status || 'Accepted';
    ...
    >
      ...
      >
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h2 className="text-lg font-display font-bold text-slate-900">
                  {submission.userName}'s Submission
                </h2>
        ...
        <div className="p-6 overflow-y-auto space-y-5 text-sm">
          {/* Failed status banner if not accepted */}
          {!isAccepted && (
            <div
            ...
            >
              ...
              <div className="text-xs space-y-1">
                <div className="font-bold">
                  LeetCode Result: {currentStatus}
                </div>
                {submission.testcasesPassed && (
                  <div className="font-mono bg-white/80 px-2 py-0.5 rounded border border-rose-200/60 inline-block">
                    {submission.testcasesPassed}
                  </div>
                )}
          ...
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-3">
            {submission.approach && (
              <div>
                <span className="text-xs font-bold uppercase tracking-wider text-slate-500 block mb-1 flex items-center gap-1.5">
                  ...
                  Algorithmic Approach / Inspection
                </span>
            ...
            <div className="flex flex-wrap items-center gap-4 pt-2 border-t border-slate-200/80 text-xs">
              {submission.runtime && (
                <div>
                  <span className="text-slate-500">Runtime / Outcome: </span>
                  <strong className="text-slate-900 font-mono">{submission.runtime}</strong>
                  {submission.runtimePercentile && (
                    <span className="ml-1 text-[11px] text-emerald-600">({submission.runtimePercentile})</span>
                  )}
                </div>
              )}
              {submission.memory && (
                <div>
                  ...
                  {submission.memoryPercentile && (
                    <span className="ml-1 text-[11px] text-emerald-600">({submission.memoryPercentile})</span>
                  )}
                </div>
              )}
              <div>
